# First Class Exotics — handoff (19 Aug 2026, end of day)
...
production on 19 Aug, not carried from the previous handoff.
...
1. ~~**#9 / #10**~~ — ✅ done 20 Aug, see §5. Two leftovers, both low priority: the
   `og:image` on **34 pages is still under 1200px wide** (mostly the legacy root
   `fleet_*.jpg` 800×600 files — every vinyl-wrap page, `index.html`, `gift.html`,
   `car-ferrari-f8.html`), and two publisher-template posts have a sub-1200px hero of their
   own (`mclaren-788hs-end-of-an-era` 1172×810, `monterey-car-week-2026` 1086×1448 portrait).
   Those two have genuine subject-matter photos — do **not** swap them for a generic fleet
   car without asking.
...
4. **Reviews** — re-run the Gmail Adobe Sign query (not done since 14 Aug), work the one-time
   renters from the `fce-review-ask-list` artifact. This is the actual business needle.
5. **#11 stock photos** — needs Ali's decision first. Do not start unilaterally.

**Do not invent site work.** After #9/#10 there is no meaningful engineering left; the
remaining upside is reviews and the stock-photo decision.
