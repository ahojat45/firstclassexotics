...
exports.handler = async function handler(event) {
  ...
  try {
    const docs = await supabaseFetch(`/rest/v1/documents?id=eq.${encodeURIComponent(documentId)}&select=id,storage_bucket,storage_path,mime_type`);
    if (!docs.length) {
    ...
    return json(200, {
      ...
      signedUrl: `${url}/storage/v1${signed.signedURL}`,
    });
  } catch (error) {
    return json(500, { error: error.message || 'Failed to create document download link' });
  }
};
